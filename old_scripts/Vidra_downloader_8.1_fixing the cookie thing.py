...
class VidraApp(ctk.CTk):
    def __init__(self):
        ...
        self.load_prefs()
        self.check_initial_cookies() # Check for cookies at startup
    ...
    def check_initial_cookies(self):
        """Prompt user for cookies.txt if missing at launch"""
    ...
    def select_cookie_file(self):
        ...
        if file_path:
            try:
                shutil.copy(file_path, "cookies.txt")
                messagebox.showinfo("Success", "Cookies file loaded successfully!")
                # Refresh engine with new cookie path if it exists
                if hasattr(self, 'engine'):
                    self.engine = DownloadEngine(os.path.abspath("cookies.txt"))
                # Refresh UI label if prefs page is open
                if hasattr(self, 'cookie_status_label'):
                    self.cookie_status_label.configure(text="Status: Found ✅", text_color=ACCENT_COLOR)
    ...
    def setup_prefs_page(self):
        ...
        # Cookie Settings
        ctk.CTkLabel(container, text="Cookie Settings", font=("Segoe UI", 14)).pack(anchor="w", padx=30, pady=(15, 5))
        cookie_exists = os.path.exists("cookies.txt")
        self.cookie_status_label = ctk.CTkLabel(container, text=f"Status: {'Found ✅' if cookie_exists else 'Not Found ❌'}", font=("Segoe UI", 11), text_color=ACCENT_COLOR if cookie_exists else DANGER_COLOR)
        ...
        # Dev Mode
        self.check_dev = ctk.CTkCheckBox(container, text="Developer Mode (Show technical IDs and Sizes)", font=("Segoe UI", 14), fg_color=ACCENT_COLOR, hover_color="#00CC88", command=self.toggle_dev_mode)
        if self.prefs["dev_mode"]: self.check_dev.select()
        self.check_dev.pack(anchor="w", padx=30, pady=(15, 20))
    ...
    def paste_and_analyze(self):
        ...
        # Signify analysis phase in terminal
    ...
    def run_fetch(self, url):
        try:
            ...
            for f in formats:
                ...
                # Format size string as requested from old version
                ...
                # Display label construction from old version logic
                display_label = ""
                ...
                # construct technical label for Dev Mode
                if is_dev:
                    final_label = f"{display_label} ({fid}) {f_size_str}".strip()
                else:
                    final_label = f"{display_label}".strip()
                
                if not final_label: continue

                # Video List (Only separate streams or combined)
                if v_codec != 'none':
                ...
                # Audio List (Only separate streams or combined in Dev Mode)
                if a_codec != 'none':
            ...
            # Sort as per old successful logic (Resolution/Bitrate descending)
            v_list.sort(key=lambda x: x[2], reverse=True)
            a_list.sort(key=lambda x: x[2], reverse=True)
            
            v_list_final = [(x[0], x[1]) for x in v_list]
            a_list_final = [(x[0], x[1]) for x in a_list]
                        
            # Pass data back through queue to update main thread safely
            self.update_queue.put(('formats_ready', 0, (v_list_final, a_list_final, info.get('thumbnail'), temp_v_map, temp_a_map)))
        except Exception as e:
