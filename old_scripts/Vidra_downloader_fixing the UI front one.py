...
class VidraApp(ctk.CTk):
    ...
    def setup_downloader_page(self):
        ...
        self.mode_switch = ctk.CTkSegmentedButton(sel_frame, values=["Video", "Audio", "Both"], variable=self.mode_var, height=45, corner_radius=12, selected_color=ACCENT_COLOR, unselected_color=BG_CARD, selected_hover_color=ACCENT_COLOR, command=self.update_ui_state)
    ...
    def run_download(self, jid, url, folder):
        try:
            ...
            # Map selected labels back to IDs if possible
            v_val = self.v_qual_var.get()
            a_val = self.a_qual_var.get()
