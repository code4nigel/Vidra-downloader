...
class DownloadCard(ctk.CTkFrame):
    def __init__(self, master, job_id, title, folder, stop_callback, **kwargs):
        ...
        actions.grid(row=0, column=2, rowspan=3, padx=15)
        
        self.folder_btn = ctk.CTkButton(actions, text="📁", width=35, height=35, corner_radius=10, fg_color="#333333", hover_color="#444444", command=self.open_folder)
        self.folder_btn.pack(pady=5)
        
        self.stop_btn = ctk.CTkButton(actions, text="🛑", width=35, height=35, corner_radius=10, fg_color="#442222", hover_color=DANGER_COLOR, command=self.stop_click)
...
class VidraApp(ctk.CTk):
    def __init__(self):
        ...
        # Maps to link UI text back to raw format IDs
        self.video_map = {}
        self.audio_map = {}
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self.page_manage, text="Downloads", font=("Segoe UI", 28, "bold")).pack(pady=(30, 10), padx=40, anchor="w")
    ...
    def setup_downloader_page(self):
        ...
        self.dl_status_header.pack(pady=20)
        ...
        self.mode_switch = ctk.CTkSegmentedButton(
            ...
            height=50, 
            corner_radius=25, 
            fg_color="#000000", # Pure black background to blend
        ...
        self.v_qual_menu = ctk.CTkOptionMenu(self.quality_frame, values=["Best Video"], variable=self.v_qual_var, fg_color=BG_CARD, button_color="#222222", height=48, corner_radius=15)
        self.v_qual_menu.pack(side="left", fill="x", expand=True, padx=(0, 7))
        self.a_qual_var = ctk.StringVar(value="Best Audio")
        self.a_qual_menu = ctk.CTkOptionMenu(self.quality_frame, values=["Best Audio"], variable=self.a_qual_var, fg_color=BG_CARD, button_color="#222222", height=48, corner_radius=15)
    ...
    def handle_start(self):
        ...
        # Get actual IDs from selection
        vid_id = self.video_map.get(v_sel, "bestvideo")
        aud_id = self.audio_map.get(a_sel, "bestaudio")

        self.job_counter += 1
        jid = self.job_counter
        card = DownloadCard(self.scroll_frame, jid, "Initializing...", folder, lambda x: None)
    ...
    def run_download(self, jid, url, folder, custom_name, vid_id, aud_id):
        try:
            ...
            if mode == "Audio":
                opts['format'] = aud_id
                opts['postprocessors'] = [{'key': 'FFmpegExtractAudio','preferredcodec': ext if ext in ['mp3','wav','m4a'] else 'mp3'}]
            elif mode == "Video":
                opts['format'] = vid_id
                # Force specific extension if requested
                if ext != 'mp4': opts['postprocessors'] = [{'key': 'FFmpegVideoConvertor', 'preferedformat': ext}]
            else:
                # Both: Combine selected video + selected audio
