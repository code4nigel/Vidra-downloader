...
class VidraApp(ctk.CTk):
    ...
    def setup_prefs_page(self):
        ...
        # Dev Mode
        self.check_dev = ctk.CTkCheckBox(container, text="Developer Mode (Show all formats)", font=("Segoe UI", 14), fg_color=ACCENT_COLOR, hover_color="#00CC88", command=self.toggle_dev_mode)
    ...
    def run_fetch(self, url):
        try:
            ...
            for f in formats:
                ...
                # Video Filter
                if vcodec != 'none':
                    res_val = f.get('height') or "???"
                    label = f"[{fid}] {res_val}p - {f['ext']} ({self.format_size(size)})" if is_dev else f"{res_val}p ({f['ext']})"
                ...
                if acodec != 'none' and (vcodec == 'none' or is_dev):
                    abr = int(f.get('abr', 0))
                    label = f"[{fid}] {abr}k - {f['ext']} ({self.format_size(size)})" if is_dev else f"{abr}k ({f['ext']})"
    ...
    def run_download(self, jid, url, folder, custom_name, vid_id, aud_id):
        try:
            ...
            if mode == "Audio":
                ...
                opts['format'] = f"{vid_id}+bestaudio/best" if vid_id != 'bestvideo' else vid_id
                if ext != 'mp4': 
                    if 'postprocessors' not in opts: opts['postprocessors'] = []
                    opts['postprocessors'].append({'key': 'FFmpegVideoConvertor', 'preferedformat': ext})
            else:
                # Merge logic to avoid "webm merging into mp4" quality loss
