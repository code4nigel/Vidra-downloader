...
# Added 'mutagen' to ensure reliable thumbnail embedding
...
class VidraApp(ctk.CTk):
    def __init__(self):
        ...
        self.video_map = {} # Maps label to {id, size}
        self.audio_map = {}
    ...
    def setup_downloader_page(self):
        ...
        # Live Info Indicator (Quality + Size)
    ...
    def setup_prefs_page(self):
        ...
        self.change_path_btn = ctk.CTkButton(container, text="Change Path", width=100, height=24, corner_radius=12, fg_color="#333333", hover_color="#444444", command=self.set_default_directory)
        self.change_path_btn.pack(anchor="w", padx=60, pady=(0, 20))

        # Media Player
        ctk.CTkLabel(container, text="Custom Media Player Executable", font=("Segoe UI", 14)).pack(anchor="w", padx=30, pady=(10, 5))
        ...
        self.change_player_btn = ctk.CTkButton(container, text="Select Player Executable", width=150, height=24, corner_radius=12, fg_color="#333333", hover_color="#444444", command=self.set_media_player)
        self.change_player_btn.pack(anchor="w", padx=60, pady=(0, 20))
    ...
    def update_pref_vars(self):
        ...
        is_on = bool(self.check_default_path.get())
        if is_on and not self.prefs.get("default_path"):
            self.set_default_directory()
        self.prefs["use_default_path"] = is_on
        self.save_prefs()
    ...
    def update_info_indicator(self):
        ...
        v_size = v_info.get('size', 0)
        a_size = a_info.get('size', 0)
        ...
        if mode == "Both":
            txt += f" (V: {self.format_size(v_size)} + A: {self.format_size(a_size)})"
    ...
    def handle_start(self):
        ...
        self.show_manage()
        threading.Thread(target=self.run_download, args=(jid, url, folder, custom_name, vid_info['id'], aud_info['id']), daemon=True).start()

    def show_job_log(self, jid):
        log_win = ctk.CTkToplevel(self)
        log_win.title(f"Terminal Log - Job #{jid}")
        log_win.geometry("700x500")
        txt = ctk.CTkTextbox(log_win, font=("Consolas", 12))
        txt.pack(fill="both", expand=True, padx=10, pady=10)
        logs = "\n".join(self.job_logs.get(jid, ["No logs available."]))
        txt.insert("0.0", logs)
    ...
    def show_global_logs(self):
        ...
        for jid, logs in self.job_logs.items():
            all_logs.append(f"--- JOB #{jid} ---")
            all_logs.extend(logs)
            all_logs.append("\n")
        
        log_win = ctk.CTkToplevel(self)
        log_win.title("Global Download Terminal")
    ...
    def play_video(self, file_path):
        ...
        if player and os.path.exists(player):
            ...
            # Fallback to system default if no player selected
    ...
    def sync_history(self):
        ...
        for jid, card in self.cards.items():
            if card.finished:
                history.append({"title": card.title, "folder": card.folder, "file_path": card.file_path})
        ...
        try:
            with open(HISTORY_FILE, "r") as f:
                for item in json.load(f):
                    ...
                    card.file_path = item.get("file_path", "")
                    card.pack(fill="x", pady=10, padx=10)
                    card.update_progress(1.0, "0", "0", size_text="Archived")
                    self.cards[jid] = card
        except: pass
        ...
        try:
            ...
            def hook(d):
                if d['status'] == 'downloading':
                    ...
                    self.update_queue.put(('progress', jid, (p, d.get('_speed_str', 'N/A'), d.get('_eta_str', 'N/A'), None, size_str)))
                elif d['status'] == 'finished':
                    self.update_queue.put(('progress', jid, (1.0, "0", "0", None, "Processing...")))
                    self.update_queue.put(('file_done', jid, d.get('filename')))

            logger = YdlLogger(self.update_queue, jid)
            self.engine.download(url, opts, hook, logger)
            self.sync_history()
